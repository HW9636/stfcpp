...
namespace stfcpp
{
	...
	template<typename T>
	inline void basic_assert(const T& expected, const T& actual, const char* file, const int& line, const std::string& message = "Expected and actual values are different")
	{
    ...
    template<typename T>
    inline void basic_assert_onfail(const T& expected, const T& actual, const char* file, const int& line, const basic_assert_fail_func& failFunc = [](){ logger::normal("On failed function triggered. If you don't want an on fail function please use STF_BASIC_ASSERT instead."); })
    {
        if (expected == actual) return;
        failFunc();
